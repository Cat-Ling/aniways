...
const getMangaInfo = async (input: { id: string }) => {
  ...
  const image = $("div.panel-story-info > div.story-info-left img").attr("src");
...
const getChapterInfo = async (input: { chapterId: string }) => {
  ...
  const prevUrl =
    $(".navi-change-chapter-btn .navi-change-chapter-btn-prev").attr("href") ??
    $(".btn-navigation-chap > a:nth-child(1)").attr("href");

  const nextUrl =
    $(".navi-change-chapter-btn .navi-change-chapter-btn-next").attr("href") ??
  ...
  const chapter = chapterList.find(c => c.id === input.chapterId);
...
export const mangaRouter = createTRPCRouter({
  ...
  getMangaInfo: publicProcedure
    ...
    .query(async ({ input }) => {
      return getMangaInfo(input);
    }),

  getChapterPages: publicProcedure
    .input(z.object({ chapterId: z.string() }))
    .query(async ({ input }) => {
      return getChapterInfo(input);
    }),
  ...
  saveToLibrary: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const chapter = await getChapterInfo({ chapterId: input.chapterId });
      ...
      if (existingMangaLibrary) {
        await ctx.db
          ...
          .set({
            ...
            chapter: String(chapter.chapter.chapter),
            page: input.page,
          })
      ...
      const mangaInfo = await getMangaInfo({ id: input.mangaId });
  ...
  getLibrary: protectedProcedure.query(async ({ ctx }) => {
    ...
    return ctx.db.query.library.findMany({
      where: orm.eq(schema.library.userId, user.id),
    });
  }),

  getCurrentMangaLibrary: protectedProcedure
    .input(
      z.object({
        mangaId: z.string(),
      })
    ...
    .query(async ({ ctx, input }) => {
      ...
      return await ctx.db.query.library.findFirst({
        where: orm.and(
          orm.eq(schema.library.userId, user.id),
          orm.eq(schema.library.mangaId, input.mangaId)
        ),
